...
class Service:
        ...
        def connect(self, baseurl):
                ...
                try:
                        xml = urllib2.urlopen(baseurl).read().decode('utf-8')           
                except: 
...
class Layer:
        ...
        def __loadTileData(self, tileRowIndex, tileColIndex) :
                ...
                #code in this function by Manfred Egger, 2020.
                # no data value is default in this version -9999.0 EGGER.
                self.__nod = -9999.0
                array = ""
                if (url.startswith('file://') == True):
                        if(url.endswith('.zip') == True):
                                zip_file_object = zipfile.ZipFile(url[7:], 'r')
                                first_file = zip_file_object.namelist()[0]
                                file = zip_file_object.open(first_file)
                                content = file.readlines()[6:]
                                array = numpy.genfromtxt(content)
                        else:
                                asc = open(url[7:], 'r')
                                array = numpy.genfromtxt(asc.readlines()[6:])
                else:    
                        if(url.endswith('.zip') == True):
                                print(url)
                                ...
                                array = numpy.genfromtxt(urllib2.urlopen(url).readlines()[6:])
                return array
        ...
        #
        # \return String in case of error, numpy data array in case of success
        def exportToAsc(self, xLowerLeft, yLowerLeft, xUpperRight, yUpperRight,path):
                if not self.isConnected() : return 'ERROR: You need to successfully connect a layer first.'
                ...
                for tileRowIndex in range(inds0['tileRowIndex'], inds1['tileRowIndex']+1) :
                        ...
                        for tileColIndex in range(inds0['tileColIndex'], inds1['tileColIndex']+1) :
                                ...
                                # print(str(rowTileData) + ', ' + str(colTileData))
                                # print(str(nrRows) + ', ' + str(nrCols))
                                inds1asc = self.__convertCoords2Idx('lower_right', xLowerLeft, yLowerLeft)
                                # Copy the data
                                values[rowValues:rowValues+nrRows,colValues:colValues+nrCols] = \
                                        tileData[rowTileData:rowTileData+nrRows,colTileData:colTileData+nrCols]
                                header = "ncols "+str(numpy.size(values,1))+"\n"
                                header += "nrows "+str(numpy.size(values,0))+"\n"
                                header += "xllcorner "+str(self.__convertIdx2Coords(inds1asc)[0]-self.__layerInfo['cellsize'])+"\n"
                                header += "yllcorner "+str(self.__convertIdx2Coords(inds1asc)[1]-self.__layerInfo['cellsize'])+"\n"
                                header += "cellsize "+str(self.__layerInfo['cellsize'])+"\n"
                                header += "NODATA_value -9999"
                                #print (header)
                                numpy.savetxt(path, values, header=header,comments='')
                                #numpy.savetxt(path, values, header=header, fmt="%1.2f")
                                #numpy.savetxt(path, values)
